...
class MaterializedQueryTestCase(TestCase):
    ...
    def test_single_delete(self):
        ...
        client.post(
            "/admin/%s/%s/outlier/delete/" % (MaterializedQuery._meta.app_label, MaterializedQuery._meta.model_name),
            data={"post": "yes"},
        )
        ...
        from django.db import connections  # noqa
        with connections["achilles"].cursor() as cursor:
            cursor.execute("SELECT COUNT(*) FROM pg_matviews")
            self.assertTupleEqual((2,), cursor.fetchone())
            cursor.execute("SELECT COUNT(*) FROM pg_matviews WHERE matviewname = 'outlier'")
            self.assertTupleEqual((0,), cursor.fetchone())

    def test_multiple_delete(self):
        client = _login_admin(self)
        client.post(
            "/admin/%s/%s/" % (MaterializedQuery._meta.app_label, MaterializedQuery._meta.model_name),
            data={"post": "yes", "action": "delete_selected", "_selected_action": ["test2", "test3"]},
        )
        ...
        with connections["achilles"].cursor() as cursor:
            cursor.execute("SELECT COUNT(*) FROM pg_matviews")
            self.assertTupleEqual((1,), cursor.fetchone())
            cursor.execute("SELECT COUNT(*) FROM pg_matviews WHERE matviewname = 'outlier'")
            self.assertTupleEqual((1,), cursor.fetchone())
    ...
    def test_add_view(self, create_task):
        client = _login_admin(self)
        client.post(
            "/admin/%s/%s/add/" % (MaterializedQuery._meta.app_label, MaterializedQuery._meta.model_name),
        ...
        create_task.assert_called_with(
            ...
            None,
            serializers.serialize("json", [MaterializedQuery(matviewname="test", definition="SELECT")]),
            [{"added": {}}]
        )
    ...
    def test_change_view(self, create_task):
        ...
        client = _login_admin(self)
        client.post(
            "/admin/%s/%s/outlier/change/" % (MaterializedQuery._meta.app_label, MaterializedQuery._meta.model_name),
        ...
        create_task.assert_called_with(
            ...
            serializers.serialize("json", [MaterializedQuery(matviewname="outlier", definition="SELECT 2")]),
...
class CeleryTasksTestCase(TransactionTestCase):
    ...
    def test_change_celery_valid_definition_change(self):
        ...
        self.assertEquals(" SELECT 2;", MaterializedQuery.objects.get(matviewname="outlier").definition)

    def test_change_celery_invalid_definition_change(self):
        task = create_materialized_view.delay(
            int(User.objects.get(username=self.admin_user).id),
            MaterializedQuery.objects.get(matviewname="outlier").to_dict(),
            serializers.serialize("json", [MaterializedQuery(matviewname="outlier", definition="SELECT a")]),
            [{"changed": {"fields": ["definition"]}}]
        )
        ...
        self.assertTrue("Error while changing the materialized view outlier in the underlying database." in task_results.result)
        self.assertEqual(3, MaterializedQuery.objects.count())
        self.assertEquals(" SELECT 1;", MaterializedQuery.objects.get(matviewname="outlier").definition)

    def test_change_celery_matviewname_change(self):
        task = create_materialized_view.delay(
            int(User.objects.get(username=self.admin_user).id),
            MaterializedQuery.objects.get(matviewname="outlier").to_dict(),
            serializers.serialize("json", [MaterializedQuery(matviewname="outlier2", definition=" SELECT 1;")]),
        ...
        try:
            self.assertEqual(" SELECT 1;", MaterializedQuery.objects.get(matviewname="outlier2").definition)
    ...
    def test_invalid_create(self):
        task = create_materialized_view.delay(
            int(User.objects.get(username=self.admin_user).id),
            None,
            serializers.serialize("json", [MaterializedQuery(matviewname="outlier2", definition="select a")]),
            [{"added": {}}]
        )
        ...
        self.assertEqual("FAILURE", task_results.status)
        self.assertTrue("Error while creating the materialized view outlier2 in the underlying database." in task_results.result)
        self.assertEqual(3, MaterializedQuery.objects.count())
